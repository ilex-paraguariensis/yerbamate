...
class JSONChecker:
    def __init__(self, *, json_template_file_name:Optional[str], json_template:Optional[JSON]):
    ...
    def __check(self, json_data: JSON, json_template: JSON):
        if isinstance(json_data, dict):
            for key in json_template:
                ...
                assert type(json_template[key]) == type(json_data[key]), '{} is not {}'.format(json_data[key], type(json_template[key]))
        elif isinstance(json_data, list):
            self.__check_list(json_data, json_template)
        else:
            assert type(json_template) == type(json_data), '{} is not {}'.format(json_data, type(json_template))
