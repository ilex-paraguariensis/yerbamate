...
class ProbablisticConvGenerator(nn.Module):
    def __init__(self, params):
        ...
        self.layers = nn.Sequential(
            ...
            ConvBlock(
                params["nc"] * 18 * mlp, params["nc"] * 12 * mlp, 4, padding="same"
            ),
            ConvBlock(
                params["nc"] * 12 * mlp, params["nc"] * 8 * mlp, 4, padding="same"
            ),
            ConvBlock(
                params["nc"] * 8 * mlp, params["nc"] * 2 * mlp, 4, padding="same"
            ),
    ...
    def forward(self, x, noiseVariance=0.001):
        ...
        x_concat = t.zeros(
            (
                x.shape[0],
                x.shape[1],
                x.shape[2] + noise_matrix_w_h,
                x.shape[3] + noise_matrix_w_h,
            )
        )
